...
waitmysql(){
        ...
        while true;
        do
                ${MYSQLDIR}mysql -h127.0.0.1 -usbtest -psbtest -Bse "SELECT 1" mysql
...
docker run --name ps1  -e MYSQL_ROOT_PASSWORD=secret --net=host -v /mnt/data/db/docker-innodb:/var/lib/mysql -v /mnt/data/vadim/servers/docker/my-inno.cnf:/etc/mysql/my.cnf -v /data/tmp:/tmp -m ${memlimit}g -d percona/percona-server:8.0.22 --innodb_buffer_pool_size=${rockscache}G
...
echo "engine: innodb"         >> $OUTDIR/params.txt
echo "tablesize: $i"         >> $OUTDIR/params.txt
        ...
        time=1800
        sysbench oltp_read_write --threads=150 --time=$time --tables=40 --table_size=${i}000000 --mysql-host=127.0.0.1 --mysql-user=root --mysql-password=secret --max-requests=0 --report-interval=1 --mysql-db=sbtest --mysql-ssl=off  --report_csv=yes --rand-type=$randtype run |  tee -a $OUTDIR/results.txt
